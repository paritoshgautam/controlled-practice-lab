...
const workUpload = (id, prompt, rubric, keywords, sample, topic, difficulty = "hard", estimatedMinutes = 4) => ({
  id,
  type: "work_upload",
  prompt,
  rubric,
...
function makeMathTest(n) {
  ...
  add(choice(`m${n}-u1-function`, `Which equation represents y = x^2 shifted right ${a} units and up ${b} units?`, [`y = (x - ${a})^2 + ${b}`, `y = (x + ${a})^2 + ${b}`, `y = (x - ${a})^2 - ${b}`, `y = x^2 - ${a} + ${b}`], 0, "A right shift is inside as x - h, and an upward shift adds k outside.", "Unit 1: Transformations", "medium", 3));
  ...
  add(choice(`m${n}-u7-sets`, `Let U = {1,2,3,4,5,6,7,8}, A = {2,4,6,8}, and B = {1,2,3,4}. What is A union B?`, ["{1,2,3,4,6,8}", "{2,4}", "{5,7}", "{1,3,5,7}"], 0, "The union contains everything in A or B, without repeats.", "Unit 7: Sets", "easy", 2));
  ...
  add(workUpload(`m${n}-u8-interpret`, `Show your work: a histogram is skewed right. Decide whether the mean or median is usually larger and justify using the shape.`, "Full credit: recognizes right skew, notes high-end tail pulls the mean upward, compares mean and median.", ["right", "tail", "mean", "median", "larger"], "In a right-skewed distribution, the long high-value tail pulls the mean upward, so the mean is usually greater than the median.", "Unit 8: Data Displays", "hard", 4));

  add(workUpload(`m${n}-mixed-adaptive`, `Show your work for one hard question from this test. Include formulas, restrictions, or probability rules where relevant.`, "Full credit: identifies a problem type, states a correct strategy/formula, and explains why that strategy fits.", ["formula", "because", "solve", "check", "strategy"], "A strong response names the topic, writes the relevant rule, substitutes carefully, and checks restrictions or reasonableness.", "Mixed Units 1-8", "hard", 5));
